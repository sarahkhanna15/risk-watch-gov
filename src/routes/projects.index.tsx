...
export const Route = createFileRoute("/projects/")({
  head: () => ({
    meta: [
      { title: "Projects — PAIMANA Infrastructure Risk Monitoring" },
      {
        name: "description",
        content:
          "Browse and filter national infrastructure projects by Critical, High, Medium and Low implementation risk with predicted cost and time overruns.",
      },
      { property: "og:title", content: "Projects — PAIMANA" },
      {
        property: "og:description",
        content:
          "Searchable, sortable and paginated risk register of monitored infrastructure projects.",
      },
    ],
  }),
...
function ProjectsPage() {
  ...
  const toggleSort = (key: SortKey) =>
    setSort((s) =>
      s.key === key
        ? { key, dir: s.dir === 1 ? -1 : 1 }
        : { key, dir: key === "name" || key === "sector" || key === "code" ? 1 : -1 },
    );
        ...
        {stats.map((s) => {
          ...
          return (
            <button
              key={s.category}
              onClick={() => {
                setActive(isActive ? null : s.category);
                resetPage();
              }}
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-4 py-3">
          <p className="text-[11px] text-muted-foreground">
            ...
            of <span className="font-mono font-semibold text-foreground">{rows.length}</span>{" "}
            projects
          </p>
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="grid size-8 place-items-center rounded-md border border-border text-muted-foreground transition-colors hover:border-primary/50 hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
            >
